    ...
    <div class="p-6">
      ...
      <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="w-full sm:w-auto">
          ...
          <p class="text-gray-600 mt-1">{{ media.total }} média(s) au total</p>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    {{ item.type_media?.nom_media || 'Non spécifié' }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900 truncate max-w-xs">
                    {{ item.contenu?.titre || 'Aucun contenu' }}
                  </div>
                </td>
        ...
        <div v-if="media.data.length === 0" class="text-center py-12">
          ...
          <p class="mt-1 text-sm text-gray-500">
            Commencez par créer un nouveau média.
          </p>
        </div>

        <!-- Pagination -->
        <div v-if="media.data.length > 0" class="bg-white px-6 py-3 border-t border-gray-200">
          <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-sm text-gray-700">
              Affichage de
              <span class="font-medium">{{ media.from }}</span>
              à
              <span class="font-medium">{{ media.to }}</span>
              sur
              <span class="font-medium">{{ media.total }}</span>
              résultats
            </div>
            ...
            <div class="flex items-center space-x-1">
              ...
              <!-- Page précédente -->
              <button
                @click="goToPage(media.current_page - 1)"
                :disabled="media.current_page === 1"
                :class="[
                  'px-3 py-1 rounded border text-sm font-medium',
                  media.current_page === 1
              ...
              <!-- Dernière page -->
              <button
                @click="goToPage(media.last_page)"
                :disabled="media.current_page === media.last_page"
                :class="[
                  'px-3 py-1 rounded border text-sm font-medium',
                  media.current_page === media.last_page
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-gray-500">
                  ...
                  <span class="font-semibold text-gray-900">{{ mediaToDelete?.chemin }}</span> ?
...
function onSearch() {
  ...
  searchTimeout = setTimeout(() => {
    router.get(route('media.index'), { search: search.value }, {
      preserveState: true,
      replace: true
    });
  }, 300);
...
function goToPage(page: number) {
  if (page < 1 || page > props.media.last_page) return;
...
function confirmDelete() {
  if (mediaToDelete.value) {
    router.delete(route('media.destroy', mediaToDelete.value.id), {
      onSuccess: () => {
        closeDeleteModal();
      }
    });
